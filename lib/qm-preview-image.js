import './style.css';
var Ir = Object.defineProperty;
var hr = (g, a, s) => a in g ? Ir(g, a, { enumerable: !0, configurable: !0, writable: !0, value: s }) : g[a] = s;
var J = (g, a, s) => (hr(g, typeof a != "symbol" ? a + "" : a, s), s);
import Ne, { PureComponent as Dr, memo as He, useState as Je, useRef as y, useEffect as V, useCallback as Qr, forwardRef as dr, useImperativeHandle as vr } from "react";
import { createPortal as mr } from "react-dom";
var B = {}, Or = {
  get exports() {
    return B;
  },
  set exports(g) {
    B = g;
  }
}, Z = {};
/**
 * @license React
 * react-jsx-runtime.production.min.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var je;
function Kr() {
  if (je)
    return Z;
  je = 1;
  var g = Ne, a = Symbol.for("react.element"), s = Symbol.for("react.fragment"), E = Object.prototype.hasOwnProperty, D = g.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactCurrentOwner, v = { key: !0, ref: !0, __self: !0, __source: !0 };
  function Y(F, C, m) {
    var K, x = {}, l = null, S = null;
    m !== void 0 && (l = "" + m), C.key !== void 0 && (l = "" + C.key), C.ref !== void 0 && (S = C.ref);
    for (K in C)
      E.call(C, K) && !v.hasOwnProperty(K) && (x[K] = C[K]);
    if (F && F.defaultProps)
      for (K in C = F.defaultProps, C)
        x[K] === void 0 && (x[K] = C[K]);
    return { $$typeof: a, type: F, key: l, ref: S, props: x, _owner: D.current };
  }
  return Z.Fragment = s, Z.jsx = Y, Z.jsxs = Y, Z;
}
var W = {};
/**
 * @license React
 * react-jsx-runtime.development.js
 *
 * Copyright (c) Facebook, Inc. and its affiliates.
 *
 * This source code is licensed under the MIT license found in the
 * LICENSE file in the root directory of this source tree.
 */
var ke;
function Mr() {
  return ke || (ke = 1, process.env.NODE_ENV !== "production" && function() {
    var g = Ne, a = Symbol.for("react.element"), s = Symbol.for("react.portal"), E = Symbol.for("react.fragment"), D = Symbol.for("react.strict_mode"), v = Symbol.for("react.profiler"), Y = Symbol.for("react.provider"), F = Symbol.for("react.context"), C = Symbol.for("react.forward_ref"), m = Symbol.for("react.suspense"), K = Symbol.for("react.suspense_list"), x = Symbol.for("react.memo"), l = Symbol.for("react.lazy"), S = Symbol.for("react.offscreen"), q = Symbol.iterator, Q = "@@iterator";
    function h(e) {
      if (e === null || typeof e != "object")
        return null;
      var r = q && e[q] || e[Q];
      return typeof r == "function" ? r : null;
    }
    var j = g.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;
    function M(e) {
      {
        for (var r = arguments.length, A = new Array(r > 1 ? r - 1 : 0), o = 1; o < r; o++)
          A[o - 1] = arguments[o];
        U("error", e, A);
      }
    }
    function U(e, r, A) {
      {
        var o = j.ReactDebugCurrentFrame, u = o.getStackAddendum();
        u !== "" && (r += "%s", A = A.concat([u]));
        var w = A.map(function(c) {
          return String(c);
        });
        w.unshift("Warning: " + r), Function.prototype.apply.call(console[e], console, w);
      }
    }
    var se = !1, ie = !1, ce = !1, ge = !1, Be = !1, _;
    _ = Symbol.for("react.module.reference");
    function ue(e) {
      return !!(typeof e == "string" || typeof e == "function" || e === E || e === v || Be || e === D || e === m || e === K || ge || e === S || se || ie || ce || typeof e == "object" && e !== null && (e.$$typeof === l || e.$$typeof === x || e.$$typeof === Y || e.$$typeof === F || e.$$typeof === C || // This needs to include all possible module reference object
      // types supported by any Flight configuration anywhere since
      // we don't know which Flight build this will end up being used
      // with.
      e.$$typeof === _ || e.getModuleId !== void 0));
    }
    function Ce(e, r, A) {
      var o = e.displayName;
      if (o)
        return o;
      var u = r.displayName || r.name || "";
      return u !== "" ? A + "(" + u + ")" : A;
    }
    function $(e) {
      return e.displayName || "Context";
    }
    function b(e) {
      if (e == null)
        return null;
      if (typeof e.tag == "number" && M("Received an unexpected object in getComponentNameFromType(). This is likely a bug in React. Please file an issue."), typeof e == "function")
        return e.displayName || e.name || null;
      if (typeof e == "string")
        return e;
      switch (e) {
        case E:
          return "Fragment";
        case s:
          return "Portal";
        case v:
          return "Profiler";
        case D:
          return "StrictMode";
        case m:
          return "Suspense";
        case K:
          return "SuspenseList";
      }
      if (typeof e == "object")
        switch (e.$$typeof) {
          case F:
            var r = e;
            return $(r) + ".Consumer";
          case Y:
            var A = e;
            return $(A._context) + ".Provider";
          case C:
            return Ce(e, e.render, "ForwardRef");
          case x:
            var o = e.displayName || null;
            return o !== null ? o : b(e.type) || "Memo";
          case l: {
            var u = e, w = u._payload, c = u._init;
            try {
              return b(c(w));
            } catch {
              return null;
            }
          }
        }
      return null;
    }
    var k = Object.assign, t = 0, n, f, G, R, ee, de, ve;
    function me() {
    }
    me.__reactDisabledLog = !0;
    function Te() {
      {
        if (t === 0) {
          n = console.log, f = console.info, G = console.warn, R = console.error, ee = console.group, de = console.groupCollapsed, ve = console.groupEnd;
          var e = {
            configurable: !0,
            enumerable: !0,
            value: me,
            writable: !0
          };
          Object.defineProperties(console, {
            info: e,
            log: e,
            warn: e,
            error: e,
            group: e,
            groupCollapsed: e,
            groupEnd: e
          });
        }
        t++;
      }
    }
    function Xe() {
      {
        if (t--, t === 0) {
          var e = {
            configurable: !0,
            enumerable: !0,
            writable: !0
          };
          Object.defineProperties(console, {
            log: k({}, e, {
              value: n
            }),
            info: k({}, e, {
              value: f
            }),
            warn: k({}, e, {
              value: G
            }),
            error: k({}, e, {
              value: R
            }),
            group: k({}, e, {
              value: ee
            }),
            groupCollapsed: k({}, e, {
              value: de
            }),
            groupEnd: k({}, e, {
              value: ve
            })
          });
        }
        t < 0 && M("disabledDepth fell below zero. This is a bug in React. Please file an issue.");
      }
    }
    var Ee = j.ReactCurrentDispatcher, we;
    function re(e, r, A) {
      {
        if (we === void 0)
          try {
            throw Error();
          } catch (u) {
            var o = u.stack.trim().match(/\n( *(at )?)/);
            we = o && o[1] || "";
          }
        return `
` + we + e;
      }
    }
    var le = !1, Ae;
    {
      var Ze = typeof WeakMap == "function" ? WeakMap : Map;
      Ae = new Ze();
    }
    function Oe(e, r) {
      if (!e || le)
        return "";
      {
        var A = Ae.get(e);
        if (A !== void 0)
          return A;
      }
      var o;
      le = !0;
      var u = Error.prepareStackTrace;
      Error.prepareStackTrace = void 0;
      var w;
      w = Ee.current, Ee.current = null, Te();
      try {
        if (r) {
          var c = function() {
            throw Error();
          };
          if (Object.defineProperty(c.prototype, "props", {
            set: function() {
              throw Error();
            }
          }), typeof Reflect == "object" && Reflect.construct) {
            try {
              Reflect.construct(c, []);
            } catch (z) {
              o = z;
            }
            Reflect.construct(e, [], c);
          } else {
            try {
              c.call();
            } catch (z) {
              o = z;
            }
            e.call(c.prototype);
          }
        } else {
          try {
            throw Error();
          } catch (z) {
            o = z;
          }
          e();
        }
      } catch (z) {
        if (z && o && typeof z.stack == "string") {
          for (var i = z.stack.split(`
`), p = o.stack.split(`
`), d = i.length - 1, O = p.length - 1; d >= 1 && O >= 0 && i[d] !== p[O]; )
            O--;
          for (; d >= 1 && O >= 0; d--, O--)
            if (i[d] !== p[O]) {
              if (d !== 1 || O !== 1)
                do
                  if (d--, O--, O < 0 || i[d] !== p[O]) {
                    var L = `
` + i[d].replace(" at new ", " at ");
                    return e.displayName && L.includes("<anonymous>") && (L = L.replace("<anonymous>", e.displayName)), typeof e == "function" && Ae.set(e, L), L;
                  }
                while (d >= 1 && O >= 0);
              break;
            }
        }
      } finally {
        le = !1, Ee.current = w, Xe(), Error.prepareStackTrace = u;
      }
      var H = e ? e.displayName || e.name : "", ye = H ? re(H) : "";
      return typeof e == "function" && Ae.set(e, ye), ye;
    }
    function We(e, r, A) {
      return Oe(e, !1);
    }
    function Ve(e) {
      var r = e.prototype;
      return !!(r && r.isReactComponent);
    }
    function te(e, r, A) {
      if (e == null)
        return "";
      if (typeof e == "function")
        return Oe(e, Ve(e));
      if (typeof e == "string")
        return re(e);
      switch (e) {
        case m:
          return re("Suspense");
        case K:
          return re("SuspenseList");
      }
      if (typeof e == "object")
        switch (e.$$typeof) {
          case C:
            return We(e.render);
          case x:
            return te(e.type, r, A);
          case l: {
            var o = e, u = o._payload, w = o._init;
            try {
              return te(w(u), r, A);
            } catch {
            }
          }
        }
      return "";
    }
    var ne = Object.prototype.hasOwnProperty, Ke = {}, Me = j.ReactDebugCurrentFrame;
    function oe(e) {
      if (e) {
        var r = e._owner, A = te(e.type, e._source, r ? r.type : null);
        Me.setExtraStackFrame(A);
      } else
        Me.setExtraStackFrame(null);
    }
    function _e(e, r, A, o, u) {
      {
        var w = Function.call.bind(ne);
        for (var c in e)
          if (w(e, c)) {
            var i = void 0;
            try {
              if (typeof e[c] != "function") {
                var p = Error((o || "React class") + ": " + A + " type `" + c + "` is invalid; it must be a function, usually from the `prop-types` package, but received `" + typeof e[c] + "`.This often happens because of typos such as `PropTypes.function` instead of `PropTypes.func`.");
                throw p.name = "Invariant Violation", p;
              }
              i = e[c](r, c, o, A, null, "SECRET_DO_NOT_PASS_THIS_OR_YOU_WILL_BE_FIRED");
            } catch (d) {
              i = d;
            }
            i && !(i instanceof Error) && (oe(u), M("%s: type specification of %s `%s` is invalid; the type checker function must return `null` or an `Error` but returned a %s. You may have forgotten to pass an argument to the type checker creator (arrayOf, instanceOf, objectOf, oneOf, oneOfType, and shape all require an argument).", o || "React class", A, c, typeof i), oe(null)), i instanceof Error && !(i.message in Ke) && (Ke[i.message] = !0, oe(u), M("Failed %s type: %s", A, i.message), oe(null));
          }
      }
    }
    var $e = Array.isArray;
    function fe(e) {
      return $e(e);
    }
    function er(e) {
      {
        var r = typeof Symbol == "function" && Symbol.toStringTag, A = r && e[Symbol.toStringTag] || e.constructor.name || "Object";
        return A;
      }
    }
    function rr(e) {
      try {
        return Fe(e), !1;
      } catch {
        return !0;
      }
    }
    function Fe(e) {
      return "" + e;
    }
    function Ge(e) {
      if (rr(e))
        return M("The provided key is an unsupported type %s. This value must be coerced to a string before before using it here.", er(e)), Fe(e);
    }
    var X = j.ReactCurrentOwner, Ar = {
      key: !0,
      ref: !0,
      __self: !0,
      __source: !0
    }, pe, Re, Ie;
    Ie = {};
    function tr(e) {
      if (ne.call(e, "ref")) {
        var r = Object.getOwnPropertyDescriptor(e, "ref").get;
        if (r && r.isReactWarning)
          return !1;
      }
      return e.ref !== void 0;
    }
    function nr(e) {
      if (ne.call(e, "key")) {
        var r = Object.getOwnPropertyDescriptor(e, "key").get;
        if (r && r.isReactWarning)
          return !1;
      }
      return e.key !== void 0;
    }
    function or(e, r) {
      if (typeof e.ref == "string" && X.current && r && X.current.stateNode !== r) {
        var A = b(X.current.type);
        Ie[A] || (M('Component "%s" contains the string ref "%s". Support for string refs will be removed in a future major release. This case cannot be automatically converted to an arrow function. We ask you to manually fix this case by using useRef() or createRef() instead. Learn more about using refs safely here: https://reactjs.org/link/strict-mode-string-ref', b(X.current.type), e.ref), Ie[A] = !0);
      }
    }
    function ar(e, r) {
      {
        var A = function() {
          pe || (pe = !0, M("%s: `key` is not a prop. Trying to access it will result in `undefined` being returned. If you need to access the same value within the child component, you should pass it as a different prop. (https://reactjs.org/link/special-props)", r));
        };
        A.isReactWarning = !0, Object.defineProperty(e, "key", {
          get: A,
          configurable: !0
        });
      }
    }
    function sr(e, r) {
      {
        var A = function() {
          Re || (Re = !0, M("%s: `ref` is not a prop. Trying to access it will result in `undefined` being returned. If you need to access the same value within the child component, you should pass it as a different prop. (https://reactjs.org/link/special-props)", r));
        };
        A.isReactWarning = !0, Object.defineProperty(e, "ref", {
          get: A,
          configurable: !0
        });
      }
    }
    var ir = function(e, r, A, o, u, w, c) {
      var i = {
        // This tag allows us to uniquely identify this as a React Element
        $$typeof: a,
        // Built-in properties that belong on the element
        type: e,
        key: r,
        ref: A,
        props: c,
        // Record the component responsible for creating this element.
        _owner: w
      };
      return i._store = {}, Object.defineProperty(i._store, "validated", {
        configurable: !1,
        enumerable: !1,
        writable: !0,
        value: !1
      }), Object.defineProperty(i, "_self", {
        configurable: !1,
        enumerable: !1,
        writable: !1,
        value: o
      }), Object.defineProperty(i, "_source", {
        configurable: !1,
        enumerable: !1,
        writable: !1,
        value: u
      }), Object.freeze && (Object.freeze(i.props), Object.freeze(i)), i;
    };
    function cr(e, r, A, o, u) {
      {
        var w, c = {}, i = null, p = null;
        A !== void 0 && (Ge(A), i = "" + A), nr(r) && (Ge(r.key), i = "" + r.key), tr(r) && (p = r.ref, or(r, u));
        for (w in r)
          ne.call(r, w) && !Ar.hasOwnProperty(w) && (c[w] = r[w]);
        if (e && e.defaultProps) {
          var d = e.defaultProps;
          for (w in d)
            c[w] === void 0 && (c[w] = d[w]);
        }
        if (i || p) {
          var O = typeof e == "function" ? e.displayName || e.name || "Unknown" : e;
          i && ar(c, O), p && sr(c, O);
        }
        return ir(e, i, p, u, o, X.current, c);
      }
    }
    var he = j.ReactCurrentOwner, xe = j.ReactDebugCurrentFrame;
    function N(e) {
      if (e) {
        var r = e._owner, A = te(e.type, e._source, r ? r.type : null);
        xe.setExtraStackFrame(A);
      } else
        xe.setExtraStackFrame(null);
    }
    var De;
    De = !1;
    function Qe(e) {
      return typeof e == "object" && e !== null && e.$$typeof === a;
    }
    function Se() {
      {
        if (he.current) {
          var e = b(he.current.type);
          if (e)
            return `

Check the render method of \`` + e + "`.";
        }
        return "";
      }
    }
    function gr(e) {
      {
        if (e !== void 0) {
          var r = e.fileName.replace(/^.*[\\\/]/, ""), A = e.lineNumber;
          return `

Check your code at ` + r + ":" + A + ".";
        }
        return "";
      }
    }
    var Le = {};
    function Br(e) {
      {
        var r = Se();
        if (!r) {
          var A = typeof e == "string" ? e : e.displayName || e.name;
          A && (r = `

Check the top-level render call using <` + A + ">.");
        }
        return r;
      }
    }
    function Ye(e, r) {
      {
        if (!e._store || e._store.validated || e.key != null)
          return;
        e._store.validated = !0;
        var A = Br(r);
        if (Le[A])
          return;
        Le[A] = !0;
        var o = "";
        e && e._owner && e._owner !== he.current && (o = " It was passed a child from " + b(e._owner.type) + "."), N(e), M('Each child in a list should have a unique "key" prop.%s%s See https://reactjs.org/link/warning-keys for more information.', A, o), N(null);
      }
    }
    function be(e, r) {
      {
        if (typeof e != "object")
          return;
        if (fe(e))
          for (var A = 0; A < e.length; A++) {
            var o = e[A];
            Qe(o) && Ye(o, r);
          }
        else if (Qe(e))
          e._store && (e._store.validated = !0);
        else if (e) {
          var u = h(e);
          if (typeof u == "function" && u !== e.entries)
            for (var w = u.call(e), c; !(c = w.next()).done; )
              Qe(c.value) && Ye(c.value, r);
        }
      }
    }
    function ur(e) {
      {
        var r = e.type;
        if (r == null || typeof r == "string")
          return;
        var A;
        if (typeof r == "function")
          A = r.propTypes;
        else if (typeof r == "object" && (r.$$typeof === C || // Note: Memo only checks outer props here.
        // Inner props are checked in the reconciler.
        r.$$typeof === x))
          A = r.propTypes;
        else
          return;
        if (A) {
          var o = b(r);
          _e(A, e.props, "prop", o, e);
        } else if (r.PropTypes !== void 0 && !De) {
          De = !0;
          var u = b(r);
          M("Component %s declared `PropTypes` instead of `propTypes`. Did you misspell the property assignment?", u || "Unknown");
        }
        typeof r.getDefaultProps == "function" && !r.getDefaultProps.isReactClassApproved && M("getDefaultProps is only used on classic React.createClass definitions. Use a static property named `defaultProps` instead.");
      }
    }
    function Cr(e) {
      {
        for (var r = Object.keys(e.props), A = 0; A < r.length; A++) {
          var o = r[A];
          if (o !== "children" && o !== "key") {
            N(e), M("Invalid prop `%s` supplied to `React.Fragment`. React.Fragment can only have `key` and `children` props.", o), N(null);
            break;
          }
        }
        e.ref !== null && (N(e), M("Invalid attribute `ref` supplied to `React.Fragment`."), N(null));
      }
    }
    function qe(e, r, A, o, u, w) {
      {
        var c = ue(e);
        if (!c) {
          var i = "";
          (e === void 0 || typeof e == "object" && e !== null && Object.keys(e).length === 0) && (i += " You likely forgot to export your component from the file it's defined in, or you might have mixed up default and named imports.");
          var p = gr(u);
          p ? i += p : i += Se();
          var d;
          e === null ? d = "null" : fe(e) ? d = "array" : e !== void 0 && e.$$typeof === a ? (d = "<" + (b(e.type) || "Unknown") + " />", i = " Did you accidentally export a JSX literal instead of a component?") : d = typeof e, M("React.jsx: type is invalid -- expected a string (for built-in components) or a class/function (for composite components) but got: %s.%s", d, i);
        }
        var O = cr(e, r, A, u, w);
        if (O == null)
          return O;
        if (c) {
          var L = r.children;
          if (L !== void 0)
            if (o)
              if (fe(L)) {
                for (var H = 0; H < L.length; H++)
                  be(L[H], e);
                Object.freeze && Object.freeze(L);
              } else
                M("React.jsx: Static children should always be an array. You are likely explicitly calling React.jsxs or React.jsxDEV. Use the Babel transform instead.");
            else
              be(L, e);
        }
        return e === E ? Cr(O) : ur(O), O;
      }
    }
    function Er(e, r, A) {
      return qe(e, r, A, !0);
    }
    function wr(e, r, A) {
      return qe(e, r, A, !1);
    }
    var lr = wr, fr = Er;
    W.Fragment = E, W.jsx = lr, W.jsxs = fr;
  }()), W;
}
(function(g) {
  process.env.NODE_ENV === "production" ? g.exports = Kr() : g.exports = Mr();
})(Or);
const Fr = "_head_eto08_1", Gr = "_operation-icon_eto08_13", pr = "_rotate90_eto08_26", Rr = "_body_eto08_29", xr = "_button_eto08_34", Sr = "_button-hover_eto08_50", Lr = "_prev-button_eto08_55", Yr = "_disabled_eto08_78", br = "_next-button_eto08_83", qr = "_bodyContent_eto08_111", yr = "_preview-img_eto08_118", jr = "_foot_eto08_124", kr = "_foot-button_eto08_134", zr = "_foot-prev-button_eto08_147", Pr = "_foot-next-button_eto08_171", Ur = "_foot-slider_eto08_195", Nr = "_foot-slider-list_eto08_203", Hr = "_foot-slider-list-item_eto08_211", Jr = "_active_eto08_233", I = {
  head: Fr,
  operationIcon: Gr,
  rotate90: pr,
  body: Rr,
  button: xr,
  buttonHover: Sr,
  prevButton: Lr,
  disabled: Yr,
  nextButton: br,
  bodyContent: qr,
  previewImg: yr,
  foot: jr,
  footButton: kr,
  footPrevButton: zr,
  footNextButton: Pr,
  footSlider: Ur,
  footSliderList: Nr,
  footSliderListItem: Hr,
  active: Jr
};
class Tr extends Dr {
  constructor(s) {
    super(s);
    J(this, "dv");
    this.dv = document.createElement("div"), document.body.appendChild(this.dv);
  }
  componentWillUnmount() {
    document.body.removeChild(this.dv), this.dv = null;
  }
  render() {
    return mr(this.props.children, this.dv);
  }
}
const Xr = "_modal-mask_1ujf1_1", Zr = "_open_1ujf1_12", Wr = "_modal-content_1ujf1_16", Vr = "_hide-animation_1ujf1_30", _r = "_show-animation_1ujf1_34", ae = {
  modalMask: Xr,
  open: Zr,
  modalContent: Wr,
  hideAnimation: Vr,
  showAnimation: _r
};
function $r(g) {
  const [a, s] = Je(!1), { open: E, onClose: D, children: v, mask: Y = !0, maskClosable: F = !0 } = g, C = y(), m = y();
  V(() => {
    E ? (document.body.style.overflow = "none", C.current.style.display = "", m.current.style.display = "", setTimeout(() => s(() => !0), 20)) : (document.body.style.overflow = "", s(() => !1), setTimeout(() => {
      C.current.style.display = "none", m.current.style.display = "none";
    }, 300));
  }, [E]);
  const K = Qr(() => {
    F && (D == null || D());
  }, [F]);
  return /* @__PURE__ */ B.jsxs(Tr, { children: [
    Y && /* @__PURE__ */ B.jsx(
      "div",
      {
        ref: C,
        onClick: K,
        style: { display: "none" },
        className: `${ae.modalMask}${a ? " " + ae.open : ""}`
      }
    ),
    /* @__PURE__ */ B.jsx(
      "div",
      {
        ref: m,
        style: { display: "none" },
        className: `${ae.modalContent}${a ? " " + ae.open : ""}`,
        children: v
      }
    )
  ] });
}
const eA = He($r);
function ze(g, a) {
  return a === null ? g : { ...g, ...a };
}
function rA(g) {
  const [a, s] = Je(g);
  function E(D) {
    s(typeof D == "function" ? (v) => ze(v, D(v)) : (v) => ze(v, D));
  }
  return [a, E];
}
const AA = "data:image/gif;base64,";
class tA {
  constructor() {
    J(this, "intersection");
    J(this, "observerCallback", (a) => {
      const s = a.length;
      for (let E = 0; E < s; E++) {
        const { target: D, intersectionRatio: v } = a[E];
        v > 0 && (this.unobserveImage(D), nA(D));
      }
    });
    J(this, "observeImage", (a) => {
      Array.isArray(a) ? a.forEach((s) => this.intersection.observe(s)) : this.intersection.observe(a);
    });
    J(this, "unobserveImage", (a) => {
      Array.isArray(a) ? a.forEach((s) => this.intersection.unobserve(s)) : this.intersection.unobserve(a);
    });
    this.intersection = new IntersectionObserver(this.observerCallback);
  }
  destroy() {
    this.intersection.disconnect();
  }
}
const Pe = new tA();
function nA(g) {
  const a = g.getAttribute("data-src"), s = new Image();
  s.src = a, s.onload = function() {
    g.src = a;
  };
}
function oA(g, a) {
  const { src: s, alt: E, className: D, style: v, ...Y } = g, F = y();
  return V(() => (Pe.observeImage(F.current), () => {
    Pe.unobserveImage(F.current);
  }), []), vr(a, () => ({
    instance: F.current
  }), []), /* @__PURE__ */ B.jsx(
    "img",
    {
      src: AA,
      style: v,
      ...Y,
      ref: F,
      "data-src": s,
      alt: E || "图片",
      className: D
    }
  );
}
const aA = dr(oA);
const P = 120, Ue = /translateX\(([\-.0-9]*)px\)/, sA = /scale\(([\-.0-9]*)\, ([\-.0-9]*)\)/, iA = /rotateZ\(([\-.0-9]*)deg\)/;
function T(g) {
  const a = g.style.transform, s = sA.exec(a), E = iA.exec(a);
  return { scaleX: Number((s == null ? void 0 : s[1]) ?? 1), scaleY: Number((s == null ? void 0 : s[2]) ?? 1), rotateZ: Number((E == null ? void 0 : E[1]) ?? 0) };
}
function cA() {
  return {
    currentIndex: 0,
    isEndPage: !1,
    isStartPage: !1
  };
}
function gA(g) {
  const { onClose: a, open: s, imgs: E, previewImgs: D = E, index: v = 0, hasPerformance: Y } = g, [F, C] = rA(cA), { currentIndex: m, isStartPage: K, isEndPage: x } = F, l = y(), S = y(), q = y(), Q = y(0), h = y(0), j = y(!1), M = y(E);
  V(() => {
    function t() {
      if (!S.current)
        return;
      const { clientWidth: n } = S.current;
      n % P > 110 ? Q.current = Math.ceil(n / P) : Q.current = Math.floor(n / P), h.current = E.length, M.current = E, h.current <= Q.current && C({ isEndPage: !0, isStartPage: !0 });
    }
    return s && (t(), j.current = !0, window.addEventListener("resize", t, !1)), () => {
      window.removeEventListener("resize", t, !1);
    };
  }, [s, E, D]), V(() => {
    if (s) {
      let t = v;
      v <= 0 ? t = 0 : v >= h.current && (t = h.current - 1), U(t, 0), C({
        currentIndex: t,
        isStartPage: t < Math.ceil(Q.current / 2) || h.current <= Q.current,
        isEndPage: t > h.current - 1 - Math.ceil(Q.current / 2) || h.current <= Q.current
      });
    }
  }, [s, v]), V(() => {
    if (!Y)
      return;
    const t = M.current[m], n = new Image();
    return n.src = t, n.onload = () => {
      l.current && (l.current.src = t);
    }, () => {
      n.onload = null;
    };
  }, [m]);
  const U = (t, n = 300) => {
    if (!q.current || !S.current || h.current <= Q.current)
      return;
    const f = Math.ceil(Q.current / 2);
    let G = 0;
    t + 1 <= f ? G = 0 : t + 1 > h.current - f ? G = h.current * P - S.current.clientWidth : G = t * P - S.current.clientWidth / 2 + P / 2;
    const R = `transform: translateX(${G * -1}px); transition: transform ${n}ms ease`;
    q.current.style.cssText = R;
  }, se = () => {
    m <= 0 || (l.current.style.transform = "scale(1, 1) rotateZ(0deg)", C((t) => {
      let n = t.currentIndex - 1;
      return n = n >= 0 ? n : 0, U(n), {
        currentIndex: n,
        isStartPage: n < Math.ceil(Q.current / 2),
        isEndPage: n > h.current - 1 - Math.ceil(Q.current / 2)
      };
    }));
  }, ie = () => {
    m >= h.current - 1 || (l.current.style.transform = "scale(1, 1) rotateZ(0deg)", C((t) => {
      let n = t.currentIndex + 1;
      return n = n >= h.current ? h.current - 1 : n, U(n), {
        currentIndex: n,
        isStartPage: n < Math.ceil(Q.current / 2),
        isEndPage: n > h.current - 1 - Math.ceil(Q.current / 2)
      };
    }));
  }, ce = (t) => {
    m !== t && (l.current.style.transform = "scale(1, 1) rotateZ(0deg)", U(t), C({
      currentIndex: t,
      isStartPage: t < Math.ceil(Q.current / 2) || h.current <= Q.current,
      isEndPage: t > h.current - 1 - Math.ceil(Q.current / 2) || h.current <= Q.current
    }));
  }, ge = () => {
    if (K)
      return;
    const t = q.current.style.transform, [, n = 0] = Ue.exec(t) ?? [];
    let f = Number(n) + Q.current * P;
    f >= 0 ? (f = 0, C({ isStartPage: !0, isEndPage: !1 })) : C({ isStartPage: !1, isEndPage: !1 });
    const G = `transform: translateX(${f}px); transition: transform .3s ease`;
    q.current.style.cssText = G;
  }, Be = () => {
    if (x)
      return;
    const t = q.current.style.transform, { clientWidth: n } = S.current, f = h.current * P - n, [, G = 0] = Ue.exec(t) ?? [];
    let R = Number(G) - Q.current * P;
    R <= -f ? (R = -f, C({ isEndPage: !0, isStartPage: !1 })) : C({ isEndPage: !1, isStartPage: !1 });
    const ee = `transform: translateX(${R}px); transition: transform .3s ease`;
    q.current.style.cssText = ee;
  }, _ = () => {
    const { scaleX: t, scaleY: n, rotateZ: f } = T(l.current);
    l.current.style.transform = `scale(${t}, ${n * -1}) rotateZ(${f}deg)`;
  }, ue = () => {
    const { scaleX: t, scaleY: n, rotateZ: f } = T(l.current);
    l.current.style.transform = `scale(${t * -1}, ${n}) rotateZ(${f}deg)`;
  }, Ce = () => {
    const { scaleX: t, scaleY: n, rotateZ: f } = T(l.current);
    l.current.style.transform = `scale(${t}, ${n}) rotateZ(${f - 90}deg)`;
  }, $ = () => {
    const { scaleX: t, scaleY: n, rotateZ: f } = T(l.current);
    l.current.style.transform = `scale(${t}, ${n}) rotateZ(${f + 90}deg)`;
  }, b = () => {
    const { scaleX: t, scaleY: n, rotateZ: f } = T(l.current);
    l.current.style.transform = `scale(${t * 1.5}, ${n * 1.5}) rotateZ(${f}deg)`;
  }, k = () => {
    const { scaleX: t, scaleY: n, rotateZ: f } = T(l.current);
    let G = t / 1.5, R = n / 1.5;
    G = G <= 1 ? 1 : G, R = R <= 1 ? 1 : R, l.current.style.transform = `scale(${G}, ${R}) rotateZ(${f}deg)`;
  };
  return !s && !j.current ? null : /* @__PURE__ */ B.jsx(eA, { open: s, children: /* @__PURE__ */ B.jsxs("div", { style: { width: "100%", height: "100%" }, children: [
    /* @__PURE__ */ B.jsxs("div", { className: I.head, children: [
      /* @__PURE__ */ B.jsx("i", { className: `qm-iconfont qm-icon-swap-outline ${I.operationIcon} ${I.rotate90}`, onClick: _ }),
      /* @__PURE__ */ B.jsx("i", { className: `qm-iconfont qm-icon-swap-outline ${I.operationIcon}`, onClick: ue }),
      /* @__PURE__ */ B.jsx("i", { className: `qm-iconfont qm-icon-rotate-left ${I.operationIcon}`, onClick: Ce }),
      /* @__PURE__ */ B.jsx("i", { className: `qm-iconfont qm-icon-rotate-right ${I.operationIcon}`, onClick: $ }),
      /* @__PURE__ */ B.jsx("i", { className: `qm-iconfont qm-icon-minus-circle ${I.operationIcon}`, onClick: k }),
      /* @__PURE__ */ B.jsx("i", { className: `qm-iconfont qm-icon-plus-circle ${I.operationIcon}`, onClick: b }),
      /* @__PURE__ */ B.jsx("i", { className: `qm-iconfont qm-icon-close ${I.operationIcon}`, onClick: a })
    ] }),
    /* @__PURE__ */ B.jsxs("div", { className: I.body, children: [
      /* @__PURE__ */ B.jsx(
        "div",
        {
          onClick: se,
          className: `${I.prevButton}${m === 0 ? " " + I.disabled : ""}`,
          children: /* @__PURE__ */ B.jsx("i", { className: "qm-iconfont qm-icon-arrow-left-bold", style: { fontSize: 60 } })
        }
      ),
      /* @__PURE__ */ B.jsx("div", { className: I.bodyContent, children: /* @__PURE__ */ B.jsx(
        "img",
        {
          ref: l,
          alt: "预览图片",
          src: D[m],
          className: I.previewImg,
          style: { transform: "scale(1, 1) rotateZ(0)" }
        }
      ) }),
      /* @__PURE__ */ B.jsx(
        "div",
        {
          onClick: ie,
          className: `${I.nextButton}${m >= h.current - 1 ? " " + I.disabled : ""}`,
          children: /* @__PURE__ */ B.jsx("i", { className: "qm-iconfont qm-icon-arrow-right-bold", style: { fontSize: 60 } })
        }
      )
    ] }),
    /* @__PURE__ */ B.jsxs("div", { className: I.foot, children: [
      /* @__PURE__ */ B.jsx("div", { className: `${I.footPrevButton}${K ? " " + I.disabled : ""}`, onClick: ge, children: /* @__PURE__ */ B.jsx("i", { className: "qm-iconfont qm-icon-arrow-left-bold", style: { fontSize: 30 } }) }),
      /* @__PURE__ */ B.jsx(
        "div",
        {
          onClick: Be,
          className: `${I.footNextButton}${x ? " " + I.disabled : ""}`,
          children: /* @__PURE__ */ B.jsx("i", { className: "qm-iconfont qm-icon-arrow-right-bold", style: { fontSize: 30 } })
        }
      ),
      /* @__PURE__ */ B.jsx("div", { className: I.footSlider, ref: S, children: /* @__PURE__ */ B.jsx("ul", { className: I.footSliderList, ref: q, children: D.map(
        (t, n) => /* @__PURE__ */ B.jsx(
          "li",
          {
            onClick: () => ce(n),
            className: `${I.footSliderListItem}${m === n ? " " + I.active : ""}`,
            children: /* @__PURE__ */ B.jsx(aA, { src: t, alt: "pic" })
          },
          `${t}~${n}`
        )
      ) }) })
    ] })
  ] }) });
}
const EA = He(gA);
export {
  EA as default
};
